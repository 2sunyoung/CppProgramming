...
실습과제2 에서는 tri1 지역객체 생성 -> tri1의 면적 출력 -> tri2 지역객체 생성 -> tri2의 면적 출력 순으로 실행되어 <br>
tri1의 생성메시지 출력 -> tri1의 면적 결과 메시지 -> tri2의 생성메시지 출력 -> tri2의 면적 결과 메시지 순으로 실행결과가 나타난다.<br><br>
그러나 실습과제3 에서는 tri1 전역객체 생성 -> tri2 전역객체 생성 -> tri2의 면적 출력 -> tri1의 면적 출력 순으로 실행되어 <br>
tri1의 생성메시지 출력 -> tri2의 생성메시지 출력 -> tri2의 면적 결과 메시지 -> tri1의 면적 결과 메시지 순으로 실행결과가 나타난다.<br><br>
...
전역 객체는 main 함수 시작 전에 초기화 작업을 할 때 생성자가 호출된다는 점에서 실습과제2와 3의 실행결과 차이가 나는 것이다.
